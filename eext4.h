...
static char eext4_placement_police (struct inode *inode, umode_t mode) {
	int pfm_level = (mode >> 16) & 0x000f;
	int journal_mode = (mode >> 20) & 0x00f;
	int is_bigalloc = (mode >> 24) & 0x0f;
	int times = 0;
	int device_index;

	while(++times <= EEXT4_ONLINE_DEVICE_NUM) {
		device_index = (EXT4_I(inode)->i_rr_counter++) % EEXT4_ONLINE_DEVICE_NUM;
		if(eext4_devices[device_index]->pfm_level == pfm_level 
			&& eext4_devices[device_index]->journal_mode == journal_mode
			&& eext4_devices[device_index]->is_bigalloc == is_bigalloc)
			return device_index;
	}

	//TODO return error
	//printk(KERN_ERR "No such partition! pmf_level %d, journal_mode %d\n", pfm_level, journal_mode);
