...
static int ext4_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	eext4_warning("ext4_fill_super : sb_block %llu", sb_block);
...
//		return ERR_CAST(bdev);
	...
	//	blkdev_put(bdev, mode);
		...
		// dulian: bd_super points to spanfs sb, not each ext4 sb
		tmp_bdev->bd_super = s;
	}
		...
		char *multi_dev_name, void *data)
{
	...
	int is_first_par;
	int pfm_level;
	char *journal_str[] = {"data=writeback", "data=ordered", "data=journal"};
	...
	/* mount format: mount -t spanfsv3 /dev/ram,/dev/ssd,/dev/sda /mnt */
	...
	while((dev_name = strsep(&multi_dev_name, ",")) != NULL) {

		//extract performance level of the device
		pfm_level = *dev_name - '0';
		dev_name++;

		printk(KERN_INFO "mount device %s level %d\n", dev_name, pfm_level);
		bdev = blkdev_get_by_path(dev_name, mode, fs_type);
		if (IS_ERR(bdev))
			return ERR_CAST(bdev);
		is_first_par = 1;
		...
		do {
			...
			eext4_devices[i]->pfm_level = pfm_level;
			eext4_devices[i]->journal_mode = i % 3;
			eext4_devices[i]->is_bigalloc = i % 2;
			eext4_devices[i]->is_first_par = is_first_par;
			is_first_par = 0;
			
			root = spanfs_mount_bdev(domain_type, flags, dev_name, journal_str[i%3], ext4_fill_super, bdev, i);
...
static void eext4_kill_block_super(struct super_block *sb) {
	...
	*/
	...
	for (i = EEXT4_ONLINE_DEVICE_NUM - 1; i >= 0; i--) {
		printk(KERN_INFO "eext4_kill_block_super: par_no %d, is_first_par %d\n", i, eext4_devices[i]->is_first_par);
		if(eext4_devices[i]->is_first_par)
			blkdev_put(eext4_devices[i]->bdev, mode | FMODE_EXCL);
...
static int __init ext4_init_fs(void)
{
	...
	if (err)
		goto out6;
	ext4_kset = kset_create_and_add("spanfsv3", NULL, fs_kobj);
	if (!ext4_kset) {
		err = -ENOMEM;
		goto out5;
	}
	ext4_proc_root = proc_mkdir("fs/spanfsv3", NULL);
...
out4:
	if (ext4_proc_root)
		remove_proc_entry("fs/spanfsv3", NULL);
